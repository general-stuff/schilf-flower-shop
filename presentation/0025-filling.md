...
We want to remove the test UI from the web app and replace it with the first real UI elements.

At the top level, we want to have a hero text, "Flower Shop". It must be visible at the top of every page.
...
* If the user clicks on a flower name, use an HTML modal dialog to display the flower details (name, description, colors). Convert markdown in the description to HTML.

## Cleanup

* Remove the demo end-to-end tests for the home page.
* Remove any demo logic from the library that might still exist.
...
As always, the code must be warning-free and tests must pass without errors/warnings. Do run the e2e tests, too.
